...
func NewAdminUseCase(repo interfaces.AdminRepository, userRepo interfaces.UserRepository, helper helper.Helper) usecase.AdminUseCase {
	return &AdminUseCase{
		repo:     repo,
		userRepo: userRepo,
		helper:   helper,
...
func (ad *AdminUseCase) Blockuser(id int) error {
	...
	exist, err := ad.userRepo.CheckUserExistByID(id)
	if err != nil {
		return err
	}

	if !exist {
		return errors.New("user with this not exist")
	}
